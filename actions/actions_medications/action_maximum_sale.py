...
class ActionMaximumSale(Action):
    def name(self) -> Text:
        return "action_maxima_venta"
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        try:
            ...
            if result:
                ...
                message = (
                    f"\U0001F4E6 *Producto más vendido:*\n"
                    f"Nombre: {producto}\n"
                    f"Cantidad total vendida: {total_vendido} unidades."
                )
                dispatcher.utter_message(text=message)
            else:
                dispatcher.utter_message(text="No se encontraron datos sobre el producto más vendido.")
